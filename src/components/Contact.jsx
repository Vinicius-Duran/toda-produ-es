import { site, whatsappLink } from "../data/site.js";
import { WhatsAppIcon, InstagramIcon } from "./icons.jsx";
import QuoteForm from "./QuoteForm.jsx";

export default function Contact() {
  return (
    <section id="orcamento" className="py-16 pb-28 lg:py-32 lg:pb-32">
      <div className="container-page">
        <div className="surface-light relative">
          <div className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-gold-500/12 blur-3xl" />
          <div className="relative grid lg:grid-cols-2">
            <div className="p-6 sm:p-10 lg:p-16">
              <span className="eyebrow">Orçamento</span>
              <h2 className="mt-6 font-display text-3xl font-semibold leading-tight tracking-tight sm:text-4xl">
                Solicite sua proposta
              </h2>
              <p className="mt-5 text-base leading-relaxed text-ink-700/80">
                Fale com a nossa equipe e receba uma proposta personalizada para a
                sua programação, com disponibilidade, valores e condições.
              </p>

              <div className="mt-9 space-y-3">
                <a
                  href={whatsappLink()}
                  target="_blank"
                  rel="noreferrer"
                  className="flex min-w-0 items-center gap-4 rounded-2xl border border-ink-900/10 bg-ink-900/[0.04] px-4 py-4 transition-colors hover:border-gold-500/50 sm:px-5"
                >
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-gold-500/15 text-gold-600">
                    <WhatsAppIcon className="h-5 w-5" />
                  </span>
                  <span className="min-w-0">
                    <span className="block text-sm text-ink-700/60">WhatsApp</span>
                    <span className="font-medium">Falar com a equipe agora</span>
                  </span>
                </a>
                <a
                  href={whatsappLink(site.whatsappPrefeituras)}
                  target="_blank"
                  rel="noreferrer"
                  className="flex min-w-0 items-center gap-4 rounded-2xl border border-ink-900/10 bg-ink-900/[0.04] px-4 py-4 transition-colors hover:border-gold-500/50 sm:px-5"
                >
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-gold-500/15 text-gold-600">
                    <WhatsAppIcon className="h-5 w-5" />
                  </span>
                  <span className="min-w-0">
                    <span className="block text-sm text-ink-700/60">Contato exclusivo para Prefeituras</span>
                    <span className="font-medium">11 99446-9433</span>
                  </span>
                </a>
                <a
                  href={`mailto:${site.email}`}
                  className="flex min-w-0 items-center gap-4 rounded-2xl border border-ink-900/10 bg-ink-900/[0.04] px-4 py-4 transition-colors hover:border-gold-500/50 sm:px-5"
                >
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-gold-500/15 text-gold-600">
                    <InstagramIcon className="h-5 w-5" />
                  </span>
                  <span className="min-w-0">
                    <span className="block text-sm text-ink-700/60">E-mail</span>
                    <span className="break-all font-medium">{site.email}</span>
                  </span>
                </a>
              </div>
            </div>

            <div className="border-t border-ink-900/10 bg-ink-900/[0.03] p-6 sm:p-10 lg:border-l lg:border-t-0 lg:p-16">
              <QuoteForm />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
